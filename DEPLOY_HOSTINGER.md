...
5. Usa `docker-compose.yml` si quieres acceso público.
6. Usa `docker-compose.private.yml` si quieres que quede privado para VPN/proxy interno.
...
- `Público con dominio`: usa Traefik y mantén `3000:3000` mientras haces pruebas.
- `Privado por VPN`: usa `docker-compose.private.yml` y publícalo detrás de tu VPN o un proxy que escuche en la red privada.
